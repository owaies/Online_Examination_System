...
<!--home start-->
<?php if(@$_GET['q']==0) {
    $result = db_query($con,"SELECT * FROM quiz where email='$email' ORDER BY date DESC") or die('Error');
...
//score details
if(@$_GET['q']== 1) {
  $q=db_query($con,"SELECT distinct q.title,u.name,u.college,h.score,h.date from user u,history h,quiz q where q.email='$email' and q.eid=h.eid and h.email=u.email order by q.eid DESC")or die('Error197');
