...
<?php if(@$_GET['fid']) {
    $eid=@$_GET['fid'];
    $result = db_query($con,"SELECT * FROM quiz WHERE eid='$eid' ") or die('Error');
    while($row = db_fetch_array($result)) {
        $title = $row['title'];
        $date = $row['date'];
...
if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) {
    ...
    $sn=@$_GET['n'];
    $total=@$_GET['t'];
    $q=db_query($con,"SELECT * FROM questions WHERE eid='$eid' AND sn='$sn' " );
    ...
    $q=db_query($con,"SELECT * FROM options WHERE qid='$qid' " );
...
if(@$_GET['q']== 'result' && @$_GET['eid']) {
    $eid=@$_GET['eid'];
    $q=db_query($con,"SELECT * FROM history WHERE eid='$eid' AND email='$email' " )or die('Error157');
    ...
    $q=db_query($con,"SELECT * FROM rank WHERE email='$email' " )or die('Error157');
    while($row=db_fetch_array($q) ) {
        $s=$row['score'];
...
<?php
if(@$_GET['q']== 2) {
    $q=db_query($con,"SELECT * FROM history WHERE email='$email' ORDER BY date DESC " )or die('Error197');
    echo  '<div class="panel" data-aos="fade-up"><div class="table-responsive"><table class="table table-striped title1" >
    ...
    while($row=db_fetch_array($q) ) {
        ...
        $r=$row['sahi'];
        $qa=$row['level'];
        $q23=db_query($con,"SELECT title FROM quiz WHERE eid='$eid' " )or die('Error208');
        while($row=db_fetch_array($q23) ) { $title=$row['title']; }
        $c++;
